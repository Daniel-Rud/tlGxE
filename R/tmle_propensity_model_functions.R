                                  ...
                                  ncores = NULL)
{
  ...
  if(ncol(exposure_data) == 1) # if no confounders of exposure
  {
    ...
    PS_mod = stats::glm(formula = A ~ 1, data = exposure_data, family = "binomial",
                 weights = obs.weights)

    PS = rep(stats::plogis(stats::coef(PS_mod)), l)
  ...
  }else if(ncol(exposure_data) == 2) # if there is just a single confounder
  {
    ...
    PS_mod = stats::glm(formula = A ~ ., data = exposure_data, family = "binomial",
                 weights = obs.weights)
  ...
  else # if there are confounders for exposure
  {
    ...
    if(!parallel)
    {
      suppressWarnings(PS_mod <- SuperLearner::SuperLearner(Y = Y, X = X,
                                              newX = X,
    ...
    }else
    {
      ...
      suppressWarnings(PS_mod <- SuperLearner::snowSuperLearner(Y = Y, X = X,
                                                  newX = X,
                                                  family = binomial,
                                                  SL.library = SL.library,
                                                  cluster = cluster,
