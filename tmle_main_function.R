                ...
                parallel = F)
{
  ...
  }else if(!is.null(propensity_formula)) # if no propensity scores provided but propensity formula provided
  {
    ...
    PS = glm(formula = propensity_formula, data = full_exposure_data, 
  ...
  }else # otherwise compute propensity scores with superlearner
  {
    PS = generate_propensity_SL(exposure_data = exposure_data, 
                                ...
                                parallel = parallel, 
                                ncores = ifelse(parallel, availableCores(), NULL)) 
  }
  ...
  if(!is.null(outcome_formula))
  {
    ...
    suppressWarnings(outcome_model <- glm(formula = outcome_formula, data = full_outcome_data, 
             weights = obs.weights, family = "binomial") )
    
    Q0AW = outcome_model %>% predict(type = "response")
    Q1W = outcome_model %>% predict(newdata = full_outcome_data %>% mutate(A = 1),   type = "response")
    Q0W = outcome_model %>% predict(newdata = full_outcome_data %>% mutate(A = 0),   type = "response")
  ...
  EY1_tmle = weighted.mean(Q1_1W, obs.weights)
  EY0_tmle = weighted.mean(Q1_0W, obs.weights)
  ...
  {
    m1 = weighted.mean(Q0$Q1W, obs.weights)
    m0 = weighted.mean(Q0$Q0W, obs.weights)
  }
